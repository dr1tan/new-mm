<template>
  <div
    ref="refElement"
    class="container-style duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] container relative isolate h-[600px] w-[380px] transition-transform will-change-transform [aspect-ratio:17/21] [contain:layout_style] [perspective:600px]"
    @pointermove="handlePointerMove"
    @pointerenter="handlePointerEnter"
    @pointerleave="handlePointerLeave"
  >
    <div
      class="duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] grid h-full origin-center overflow-hidden rounded-lg border border-markajneutral-500 transition-transform will-change-transform [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.6]"
    >
      <div
        class="grid size-full mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))] [grid-area:1/1]"
      >
        <div :class="cn('size-full bg-markajpink-500', props.class)">
          <slot />
        </div>
      </div>
      <div
...
.background-style {
  ...
  --rainbow: repeating-linear-gradient(
      0deg,
      var(--color-markajpink) calc(var(--step) * 1),
      var(--color-markajgold) calc(var(--step) * 2),
      rgba(168, 255, 95, 1) calc(var(--step) * 3),
      rgba(131, 255, 247, 1) calc(var(--step) * 4),
      var(--color-markajneutral) calc(var(--step) * 5),
      var(--color-markajpink) calc(var(--step) * 6)
    )
    0% var(--bg-y) / 200% 700% no-repeat;
  --diagonal: repeating-linear-gradient(
      128deg,
      var(--color-markajneutral) 0%,
      var(--color-markajgold) 5%,
      var(--color-markajneutral) 10%,
      var(--color-markajpink) 15%
    )
